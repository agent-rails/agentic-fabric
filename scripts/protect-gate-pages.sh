...
#   ~/pr-reviewer/wiki/conventions/**   — the review rubric (cascade policy,
...
if ! command -v jq >/dev/null 2>&1; then
  ...
  case "$input" in
    *"/pr-reviewer/wiki/conventions/"*|*"/.claude/shared-wiki/"*)
      unlock_fresh && exit 0
...
case "$fp" in
  "$HOME/pr-reviewer/wiki/conventions/"*|"$HOME/.claude/shared-wiki/"*)
    unlock_fresh && exit 0
