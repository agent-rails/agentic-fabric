---
name: draft-pr-fixes
description: Draft cross-vendor patches addressing findings from a prior PR review. Spawns the your-patch-drafter agent (codex-backed, read-only sandbox) to produce one unified-diff per finding. Returns the patches for human cherry-pick — never auto-applies. Use after your-pr-reviewer/your-cross-vendor-reviewer review when the user wants candidate patches drafted. Trigger phrases include "draft fixes", "draft patches", "your-patch-drafter fix", "draft the fixes for this PR".
argument-hint: "<pr-url or owner/repo#number>"
---

Draft candidate patches for a PR using the `your-patch-drafter` agent (codex-backed cross-vendor patch drafter).

## Contract

- **Input**: PR reference + structured findings from a prior review (your-pr-reviewer + your-cross-vendor-reviewer output)
...
If invoked immediately after `/review-pr` in the same session, use the findings from that review. Otherwise, ask the user to paste the findings or rerun `/review-pr` first — your-patch-drafter needs structured findings as input, not just a PR.
...
2. **Prefetch PR diff** (orchestrator-side, before spawning your-patch-drafter):
   ...
   - Otherwise fetch:
     ...
     ```
   - Pass both to your-patch-drafter as a `PR_PREFETCH:` block in the prompt. Saves 5–15s on your-patch-drafter cold start by avoiding the redundant `gh pr diff` call inside the agent.

3. **Spawn your-patch-drafter** with this prompt:
...
3. Construct UNPRIMED prompt for codex (no your-pr-reviewer/your-cross-vendor-reviewer attribution)
...
6. Return structured YOUR_PATCH_DRAFTER_DRAFT output
...
3. **Present the drafts to the user.** For each patch:
   ...
   - Severity + file:line
   - Why your-patch-drafter declined to patch (out of scope / ambiguous / requires architectural decision)
...
5. **Apply chosen patches** (Claude does this — your-patch-drafter never writes files):
...
Typical flow:
1. `/review-pr <url>` → your-pr-reviewer (+ optionally your-cross-vendor-reviewer) returns findings
2. User decides whether to post the review and/or draft fixes
3. `/draft-pr-fixes <url>` → your-patch-drafter drafts patches based on the in-session findings
4. User cherry-picks
