...
Always print this block, regardless of tier. Two parts: a human-readable section, then a single fenced machine-readable JSON object (pr-reviewer-fenced with `pr-sizer` so callers can locate it unambiguously).
