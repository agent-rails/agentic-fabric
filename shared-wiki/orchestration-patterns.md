...
**Addendum — the continuation record (infrastructure, not a new pattern).** One narrow slice of the resume-safety above is now mechanized (see [ADR-0011](../docs/decisions/0011-continuation-record.md)). It is infrastructure *supporting* this pattern, not a pattern of its own — do not catalog it separately. A PostToolUse hook on the `Agent` tool auto-creates an ephemeral `.agents/tasks/<agentId>.json` record at dispatch time (keyed on the spawned `agentId`, which the dispatch response carries even for background `async_launched` calls), capturing the goal and any `HARD CONSTRAINTS:` section stated structurally in the dispatch prompt. On resume, a PreToolUse hook on `SendMessage` **fails loud in one narrow window only** — a record exists but its constraints are empty — blocking with "no recorded constraints … verify before acting." Every other case (no record, or constraints present) fails open, so this does not retroactively break existing name-based resumes.

What this does and does not change: it closes the "record never got created" failure mode (creation is mechanized, not a remembered step) and the "a constraint stated once early got silently dropped on resume" one. It is still **not** the task runner the paragraph above calls for — no table, no lineage, no re-dispatch, no reaper for orphaned records, and a resume performed by *name* rather than `agentId` still misses the lookup. Records are gitignored and local. The honest next step for a fleet that outgrows one operator's memory remains a real runner; this record is the minimal safety floor under the current pattern, not that runner.
