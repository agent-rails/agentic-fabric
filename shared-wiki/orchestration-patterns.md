...
The governing rule: **the user (or a slash command) is the orchestrator. Personas do not invoke other personas.** Skills are mandatory hops inside a persona's workflow. The cascade exception is documented at the bottom — pr-reviewer cascades to cross-vendor-reviewer/architect-reviewer/ai-architect as Pattern 3 (parallel fan-out with synthesizer), not as Anti-pattern B.
...
**Examples:**
- "Review this PR" → `pr-reviewer` (single-pass mode)
- "Triage messages" → `triage-agent`
...
```
/review-pr → pr-reviewer (with PR context + wiki) → report
```
...
```
                         ┌─→ architect-reviewer ─┐
pr-reviewer → fan out  ─────┼─→ ai-architect       ─┤→ pr-reviewer (synthesizer) → verdict
...
**Examples in this stack:** pr-reviewer's PR-review cascade (pr-reviewer = synthesizer; architect-reviewer + ai-architect + cross-vendor-reviewer = peer reviewers).
...
```
/work → router-persona → "this needs a review" → pr-reviewer → router (paraphrases) → user
```
...
A `pr-reviewer` that internally invokes `triage-agent` when it sees a Slack reference, with no merge step.
...
**What to do instead:** have the calling persona *recommend* a follow-up in its report. The user or a slash command runs the second pass. (PR-reviewer's cascade is *not* this anti-pattern — pr-reviewer acts as synthesizer; see stack mapping below.)
...
- Removes user agency at exactly the points where judgment matters most (e.g. cherry-picking which patch-drafter patches to apply)
...
`/review-pr` calls a `pre-review-coordinator` that calls a `quality-coordinator` that calls `pr-reviewer`.
...
**What to do instead:** keep the orchestration depth at most 1 (slash command → personas). The synthesis happens in one designated agent (the slash command, the main session, or one synthesizer like pr-reviewer).
...
|---------|-------|------|---------------|
| `pr-reviewer` | opus | PR-review synthesizer + DevSecOps lens | **Synthesizer in Pattern 3.** Cascades to peers (`architect-review`, `ai-architect`, `cross-vendor-reviewer`) on security/architectural-impact paths, then merges. Cycle-bounded (3-cap, severity gating, convergence detection). |
| `architect-review` | opus | Architectural consistency reviewer | Pattern 3 peer (called by pr-reviewer). Direct invocation also valid. |
| `ai-architect` | opus | LLM/agent-impact reviewer | Pattern 3 peer (called by pr-reviewer) on AI-impact paths. |
| `cross-vendor-reviewer` | opus | Cross-vendor reviewer (codex-backed) | Pattern 3 peer (called by pr-reviewer) for vendor diversity. **Never invoked directly by orchestrator** — by design. |
| `patch-drafter` | opus | Cross-vendor patch drafter (codex-backed) | Pattern 4 — user runs `/review-pr` then `/draft-pr-fixes`. Read-only; never applies. |
| `triage-agent` | opus | Multi-channel triage / chief of staff | Pattern 2 (`/triage`). Fans out internally to `triage-fetcher` (Pattern 5: research isolation per channel). |
| `triage-fetcher` | haiku | Per-channel message fetcher | Pattern 5 (research isolation). Async fire-and-forget per channel. Haiku is correct here — input large, output a digest. |
| `triage-reporter` | sonnet | Daily/weekly report generator | Pattern 2 internals. Not user-facing. |
| `triage-scribe` | sonnet | Triage-agent wiki maintainer | Post-task scribe. |
| `pr-review-fetcher` | haiku | PR diff/comment fetcher | Pattern 5 (research isolation). Async fire-and-forget. Haiku is correct here. |
| `pr-review-scribe` | sonnet | PR-reviewer wiki maintainer | Post-task scribe. |
...
- Every Pattern 3 peer MUST set `model:` ≥ the synthesizer's tier. If pr-reviewer is opus, every peer must be opus.
- Pattern 5 (research isolation) MAY use haiku — fetchers consume large input and return small digests, so reasoning depth matters less than throughput. Haiku is correct for `triage-fetcher`, `pr-review-fetcher`, and built-in `Explore`.
...
**Audit (2026-05-08):** Every pr-reviewer cascade peer (`architect-review`, `ai-architect`, `cross-vendor-reviewer`) is on opus. Synthesizer (`pr-reviewer`) on opus. Gate satisfied. Re-audit when adding any cascade peer.
...
### Cascade clarification (pr-reviewer ≠ Anti-pattern B)

PR-reviewer calling cross-vendor-reviewer + architect-reviewer + ai-architect *looks* like Anti-pattern B (persona-calls-persona) but is Pattern 3 (parallel fan-out with synthesizer):

- Peers run **in parallel**, not chained
- PR-reviewer **synthesizes** all peer outputs into one verdict — peers don't synthesize each other
...
Currently no production use. Reserve for competing-hypothesis debugging where teammates need to message each other (e.g. intermittent prod issue with multiple plausible root causes). For verdict on a known artifact, stick with pr-reviewer's cascade.
