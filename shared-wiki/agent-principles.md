...
Five principles that bind every agent in this workspace (your-pr-reviewer, your-cross-vendor-reviewer, your-patch-drafter, your-triage-agent, and any future agents) and my own coding work. When a parent invokes a subagent, the prompt should reference or inline these so the agent does not optimize for the wrong axis.
...
- **Reviewers (your-pr-reviewer, your-cross-vendor-reviewer):** when surfacing findings, label same-class follow-ups explicitly so the user can decide to expand the PR scope. Apply principle 2 (would a senior reviewer wave this through?) as the gate verdict.
- **Drafters (your-patch-drafter):** when finding 1 and finding 2 overlap, prefer a combined patch over two patches that conflict on apply. Surface the overlap, do not silently produce conflicting drafts.
...
Before any agent in this workspace introduces a workflow that calls another agent, see [orchestration-patterns.md](orchestration-patterns.md). Governing rule: **the user (or a slash command) is the orchestrator. Personas do not invoke other personas, except as parallel peers feeding a synthesizer (Pattern 3).** your-pr-reviewer's PR-review cascade is the canonical Pattern-3 instance. Anything that looks like a router persona, a paraphrasing chain, or a deep persona tree is Anti-pattern A/B/C/D — refactor to a slash command + user-driven sequence.
