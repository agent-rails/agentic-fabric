...
3. **Add a different model family + bound the loop (chosen).** A codex-backed peer (`spock`) reviews alongside the Claude-family peers, and the whole cascade is capped by a convergence mechanism.

## Decision

Option 3, structured as Pattern 3 (parallel fan-out with synthesizer):

- On security/architectural/AI-impact paths, `sentinel` fans out **in parallel** to `architect-review` (architecture), `ai-architect` (LLM/agent-impact), and `spock` (cross-vendor, codex-backed — a *different model family* for uncorrelated blind spots).
- `sentinel` is the **synthesizer**: it merges all peer findings into one verdict, applying evidence-calibrated severity — an unverified BLOCKER gets downgraded, because "verified by reasoning" is not verification.
...
- `spock` is **never invoked directly by the orchestrator** — only by sentinel, only when a path is genuinely security-critical. `scotty` is its counterpart on the fix side: it *drafts* cross-vendor patches (via `/draft-pr-fixes`) and never applies them.
