...
3. **Add a different model family + bound the loop (chosen).** A codex-backed peer (`your-cross-vendor-reviewer`) reviews alongside the Claude-family peers, and the whole cascade is capped by a convergence mechanism.

## Decision

Option 3, structured as Pattern 3 (parallel fan-out with synthesizer):

- On security/architectural/AI-impact paths, `your-pr-reviewer` fans out **in parallel** to `architect-review` (architecture), `ai-architect` (LLM/agent-impact), and `your-cross-vendor-reviewer` (cross-vendor, codex-backed — a *different model family* for uncorrelated blind spots).
- `your-pr-reviewer` is the **synthesizer**: it merges all peer findings into one verdict, applying evidence-calibrated severity — an unverified BLOCKER gets downgraded, because "verified by reasoning" is not verification.
...
- `your-cross-vendor-reviewer` is **never invoked directly by the orchestrator** — only by your-pr-reviewer, only when a path is genuinely security-critical. `your-patch-drafter` is its counterpart on the fix side: it *drafts* cross-vendor patches (via `/draft-pr-fixes`) and never applies them.
