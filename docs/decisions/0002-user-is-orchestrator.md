...
- The constraint is **prompt-enforced, not platform-enforced.** An earlier version of this record claimed Claude Code made it impossible "by construction" for subagents to spawn subagents — that is empirically false; nested subagent spawning has been observed directly. The router-persona, persona-calls-persona, and deep-tree anti-patterns are held off by convention, review, and the pattern catalog — not by the harness refusing to load them. The rule above (depth-1 plus the one orchestrator exception) is right; only the belief that the platform guaranteed it was wrong. See [ADR-0008](0008-orchestrator-depth-gate.md) for why the gap is documented rather than mechanized.
...
- your-pr-reviewer's cascade *looks* like persona-calls-persona but isn't: peers run in parallel, your-pr-reviewer synthesizes, the cascade is cycle-bounded, and each peer has a distinct lens. The distinction is spelled out in [orchestration-patterns.md](../../shared-wiki/orchestration-patterns.md#cascade-clarification-your-pr-reviewer--anti-pattern-b) so the exception can't be cargo-culted into a real violation.
