...
- PR-reviewer's cascade *looks* like persona-calls-persona but isn't: peers run in parallel, pr-reviewer synthesizes, the cascade is cycle-bounded, and each peer has a distinct lens. The distinction is spelled out in [orchestration-patterns.md](../../shared-wiki/orchestration-patterns.md#cascade-clarification-pr-reviewer--anti-pattern-b) so the exception can't be cargo-culted into a real violation.
