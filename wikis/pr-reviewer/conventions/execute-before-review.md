...
- **Egress / data sensitivity** — codex would see private content not approved for external review (cross-vendor-reviewer's egress screen handles this automatically; document in the review if non-execution was due to egress).
...
- **PR-reviewer** (`agents/pr-reviewer.md`) — Step 3 (Analyze) runs the relevant execute pass before producing own findings. If execution is skipped, surface the reason in the review header.
- **Cross-vendor-reviewer** (`agents/cross-vendor-reviewer.md`) — codex prompt includes "execute first" guidance; codex's sandbox supports `helm`, `terraform`, `terragrunt`, `pytest`, `yarn test` etc. in read-only mode.
