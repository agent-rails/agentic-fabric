# Voltage Wiki — Purpose

This file declares **why** the voltage wiki exists. For **how** the wiki is structured and maintained, see `schema.md`.

Every voltage and voltage-scribe invocation reads this file alongside `schema.md` so the agent knows what to flag, what to ingest deeply, and what to let slide.

## Core purpose

Voltage is the persistent memory of a Principal DevOps / SRE engineer's communication and operations surface — Slack, email, calendar, incidents, alerts, and the people behind them. It exists to compound institutional knowledge over months and years so that:
...
If voltage cannot answer these in <30 seconds from the wiki, the wiki has failed its purpose:
...
- **Not a Notion replacement.** Notion remains the home for human-shaped long-form content (RFCs, design docs, meeting notes, onboarding). Voltage points AT Notion when relevant; it does not duplicate Notion.
- **Not an autonomous decision-maker.** Voltage drafts, classifies, and surfaces. The user decides. Anything reaching master / shared infra / external messages requires human-in-the-loop.
...
- **Shared facts about people / repos / projects belong in `~/.claude/shared-wiki/`**, not duplicated here — voltage links out, doesn't fork the source of truth

## Promotion path

Facts in voltage that become cross-cutting (multiple agents need them) are candidates for promotion to `~/.claude/shared-wiki/`. Promotion requires explicit user direction — voltage-scribe never writes to shared-wiki silently. When a promotion candidate appears (e.g., a stable cross-cutting fact about a person, repo, or project), voltage surfaces it in the triage output for the user to confirm.
...
2. `~/voltage/purpose.md` (this file)
3. `~/voltage/schema.md`
4. `~/voltage/wiki/{type}/{entity}.md` — the specific entity page
