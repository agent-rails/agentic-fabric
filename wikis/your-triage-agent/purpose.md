# your-triage-agent Wiki — Purpose

This file declares **why** the your-triage-agent wiki exists. For **how** the wiki is structured and maintained, see `schema.md`.

Every your-triage-agent and your-triage-scribe invocation reads this file alongside `schema.md` so the agent knows what to flag, what to ingest deeply, and what to let slide.

## Core purpose

your-triage-agent is the persistent memory of a Principal DevOps / SRE engineer's communication and operations surface — Slack, email, calendar, incidents, alerts, and the people behind them. It exists to compound institutional knowledge over months and years so that:
...
If your-triage-agent cannot answer these in <30 seconds from the wiki, the wiki has failed its purpose:
...
- **Not a Notion replacement.** Notion remains the home for human-shaped long-form content (RFCs, design docs, meeting notes, onboarding). your-triage-agent points AT Notion when relevant; it does not duplicate Notion.
- **Not an autonomous decision-maker.** your-triage-agent drafts, classifies, and surfaces. The user decides. Anything reaching master / shared infra / external messages requires human-in-the-loop.
...
- **Shared facts about people / repos / projects belong in `~/.claude/shared-wiki/`**, not duplicated here — your-triage-agent links out, doesn't fork the source of truth

## Promotion path

Facts in your-triage-agent that become cross-cutting (multiple agents need them) are candidates for promotion to `~/.claude/shared-wiki/`. Promotion requires explicit user direction — your-triage-scribe never writes to shared-wiki silently. When a promotion candidate appears (e.g., a stable cross-cutting fact about a person, repo, or project), your-triage-agent surfaces it in the triage output for the user to confirm.
...
2. `~/your-triage-agent/purpose.md` (this file)
3. `~/your-triage-agent/schema.md`
4. `~/your-triage-agent/wiki/{type}/{entity}.md` — the specific entity page
