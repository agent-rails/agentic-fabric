...
- `wiki/log.jsonl` is append-only — one JSON object per scribe pass, written in the same Edit pass as the matching `log.md` row. Schema lives in your-triage-scribe agent prompt (controlled vocab for `op` and `source`, integer counts).
