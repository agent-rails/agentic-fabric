...
export TRIAGE_AGENT_NO_DRAIN=1

LOGDIR="$HOME/.claude/agents/triage-reporter/logs"
mkdir -p "$LOGDIR"

TARGET_DATE="${1:-$(date +%Y-%m-%d)}"
DOW=$(date -j -f "%Y-%m-%d" "$TARGET_DATE" +%u 2>/dev/null || date -d "$TARGET_DATE" +%u)

DAILY_FILE="$HOME/triage-agent/wiki/reports/daily/${TARGET_DATE}.md"
WEEKLY_FILE="$HOME/triage-agent/wiki/reports/weekly/${TARGET_DATE}.md"
...
if [ "$IS_FRIDAY" = "true" ]; then
    GEN_PROMPT="Generate a weekly report for TARGET_DATE: ${TARGET_DATE}. This includes today's daily report plus the weekly summary. Read user identity from user memory. Save daily to ~/triage-agent/wiki/reports/daily/${TARGET_DATE}.md and weekly to ~/triage-agent/wiki/reports/weekly/${TARGET_DATE}.md. Do NOT attempt to DM via Slack — orchestrator handles delivery. Git commit in ~/triage-agent/."
else
    GEN_PROMPT="Generate a daily report for TARGET_DATE: ${TARGET_DATE}. Read user identity from user memory. Save to ~/triage-agent/wiki/reports/daily/${TARGET_DATE}.md. Do NOT attempt to DM via Slack — orchestrator handles delivery. Git commit in ~/triage-agent/."
fi
...
claude --agent triage-reporter --print --permission-mode auto --max-budget-usd 5 -p "$GEN_PROMPT" >> "$LOGDIR/run.log" 2>&1
