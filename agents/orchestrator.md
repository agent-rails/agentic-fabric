...
| pr-reviewer | Principal DevSecOps Architect review — plan_validation (pre-code) OR pr_review (post-PR / pre-PR) | BEFORE implementation on DevOps/security paths; AFTER commit / BEFORE PR creation; AFTER PR exists via `/review-pr` |
...
**Note on cross-vendor review**: PR-reviewer internally cascades to `cross-vendor-reviewer` (a utility agent wrapping the OpenAI Codex CLI) on security-critical paths. The orchestrator does NOT invoke cross-vendor-reviewer directly. PR-reviewer synthesizes cross-vendor-reviewer's findings with its own wiki-backed review and returns a single verdict.
...
**Route to `pr-reviewer` (plan_validation)** when ANY of these match:
...
When the path is **security-critical** (a subset pr-reviewer determines internally — hooks, auth, secrets, prod-IaC, classifier engine), pr-reviewer automatically cascades to `cross-vendor-reviewer` for cross-vendor verification. The orchestrator does NOT need to route to cross-vendor-reviewer; pr-reviewer handles the cascade. PR-reviewer returns ONE synthesized verdict that includes both perspectives.
...
### To PR-reviewer (Plan Validation — DevOps/security paths)

Use when the Plan-Validation Routing matched. PR-reviewer operates in `plan_validation` mode — read-only on the wiki, no scribe.

```
TASK: <what we plan to implement>
TARGET_PATHS: <bulleted list of files the plan will touch — so pr-reviewer can load the right wiki pages>
REPO: <owner/repo>
...
PR-reviewer returns a `PLAN_REVIEW:` block with verdict + conditions + blast_radius + rollback_path + wiki_references. Gate on it exactly like architect-reviewer output. If approved with conditions, carry them into the ARCH_VALIDATION field passed to implementer.

**Cross-vendor cascade**: When pr-reviewer determines the path is security-critical, pr-reviewer internally invokes `cross-vendor-reviewer` (codex CLI wrapper) for an unprimed second-opinion review. PR-reviewer synthesizes cross-vendor-reviewer's findings with its own and returns ONE `PLAN_REVIEW:` block that includes a `cross_vendor_signal` field summarizing convergence/divergence. The orchestrator gates on that single synthesized verdict — no separate cross-vendor delegation needed at this layer.

### Dedup Hook for Post-PR Review

When a PR is opened and pr-reviewer was previously invoked in `plan_validation` for the same change, pass the prior verdict into the `pr_review` invocation so pr-reviewer can run a FIX-UP VERIFIED pass instead of a full review:
...
- Route to `architect-reviewer` (app code), `pr-reviewer` (DevOps paths), or BOTH in parallel for mixed changes
...
- IF any commit in the branch touched a security-critical path (hooks, auth, secrets, prod-IaC, classifier engine) → invoke `pr-reviewer` in `pr_review` mode against the diff vs base branch. PR-reviewer internally cascades to cross-vendor-reviewer for cross-vendor verification on the diff.
- PR-reviewer returns a single `PR_REVIEW:` block with verdict + findings + `cross_vendor_signal` (convergence/divergence with cross-vendor-reviewer).
- Synthesize:
  ...
  - Cross-vendor-reviewer unavailable (CLI missing / quota / egress block / timeout) → pr-reviewer proceeds on its own verdict and flags to user that cross-vendor review did not run
...
| Plan review approval | architect-reviewer and/or pr-reviewer approved per routing | Loop back to planning |
...
| Pre-PR review | PR-reviewer approves diff (cascades to cross-vendor-reviewer on security-critical paths) | Surface findings; require explicit user override to bypass |
