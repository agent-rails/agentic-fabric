...
| sentinel | Principal DevSecOps Architect review — plan_validation (pre-code) OR pr_review (post-PR / pre-PR) | BEFORE implementation on DevOps/security paths; AFTER commit / BEFORE PR creation; AFTER PR exists via `/review-pr` |
...
**Note on cross-vendor review**: Sentinel internally cascades to `spock` (a utility agent wrapping the OpenAI Codex CLI) on security-critical paths. The orchestrator does NOT invoke spock directly. Sentinel synthesizes spock's findings with its own wiki-backed review and returns a single verdict.
...
**Route to `sentinel` (plan_validation)** when ANY of these match:
...
When the path is **security-critical** (a subset sentinel determines internally — hooks, auth, secrets, prod-IaC, classifier engine), sentinel automatically cascades to `spock` for cross-vendor verification. The orchestrator does NOT need to route to spock; sentinel handles the cascade. Sentinel returns ONE synthesized verdict that includes both perspectives.
...
### To Sentinel (Plan Validation — DevOps/security paths)

Use when the Plan-Validation Routing matched. Sentinel operates in `plan_validation` mode — read-only on the wiki, no scribe.

```
TASK: <what we plan to implement>
TARGET_PATHS: <bulleted list of files the plan will touch — so sentinel can load the right wiki pages>
REPO: <owner/repo>
...
Sentinel returns a `PLAN_REVIEW:` block with verdict + conditions + blast_radius + rollback_path + wiki_references. Gate on it exactly like architect-reviewer output. If approved with conditions, carry them into the ARCH_VALIDATION field passed to implementer.

**Cross-vendor cascade**: When sentinel determines the path is security-critical, sentinel internally invokes `spock` (codex CLI wrapper) for an unprimed second-opinion review. Sentinel synthesizes spock's findings with its own and returns ONE `PLAN_REVIEW:` block that includes a `cross_vendor_signal` field summarizing convergence/divergence. The orchestrator gates on that single synthesized verdict — no separate cross-vendor delegation needed at this layer.

### Dedup Hook for Post-PR Review

When a PR is opened and sentinel was previously invoked in `plan_validation` for the same change, pass the prior verdict into the `pr_review` invocation so sentinel can run a FIX-UP VERIFIED pass instead of a full review:
...
- Route to `architect-reviewer` (app code), `sentinel` (DevOps paths), or BOTH in parallel for mixed changes
...
- IF any commit in the branch touched a security-critical path (hooks, auth, secrets, prod-IaC, classifier engine) → invoke `sentinel` in `pr_review` mode against the diff vs base branch. Sentinel internally cascades to spock for cross-vendor verification on the diff.
- Sentinel returns a single `PR_REVIEW:` block with verdict + findings + `cross_vendor_signal` (convergence/divergence with spock).
- Synthesize:
  ...
  - Spock unavailable (CLI missing / quota / egress block / timeout) → sentinel proceeds on its own verdict and flags to user that cross-vendor review did not run
...
| Plan review approval | architect-reviewer and/or sentinel approved per routing | Loop back to planning |
...
| Pre-PR review | Sentinel approves diff (cascades to spock on security-critical paths) | Surface findings; require explicit user override to bypass |
